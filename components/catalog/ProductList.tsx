...
export default function ProductList({ products, media_url }: ProductListProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent, product: any) => {
    ...
    if (product.price <= 0 || product.stock_qty === 0) return;
  ...
  return (
    <div className="space-y-4">
      {products.map((product, index) => {
        ...
        const rawPath = product.images?.[0]?.url;
        ...
        return (
          ...
          >
            ...
            <div className="relative w-full sm:w-40 h-40 flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden">
              {imageUrl ? (
                <ProductImage src={imageUrl} alt={product.name_fr} priority={index < 2} />
              ) : (
            ...
            <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                    {product.specs.processor && (
                      <span className="flex items-center gap-1">
                        ...
                        {product.specs.processor.split(' ').slice(0, 2).join(' ')}
                      </span>
                    )}
                    {product.specs.ram && (
                      <span className="flex items-center gap-1">
              ...
              <div className="flex-shrink-0 flex flex-col items-end gap-2">
                {hasValidPrice ? (
                  ...
                  <div className="text-sm text-gray-400 font-medium">Prix non disponible</div>
                )}
...
function ProductImage({
  ...
  useEffect(() => {
    if (priority) { setShouldLoad(true); return; }
