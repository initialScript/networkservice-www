...
export default function ProductCard({ product, media_url, priority = false }: Props) {
  ...
  const getImagePath = (): string | undefined => {
    ...
    if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
      try {
        return new URL(imagePath).pathname;
      } catch {
        return imagePath;
      }
    }
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    if (isAdded || !hasValidPrice || isOutOfStock) return;
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-2 sm:p-3 gap-1.5 sm:gap-2">
        ...
        {hasValidPrice ? (
          ...
          <div className="text-sm sm:text-base text-gray-400 font-medium">Prix non disponible</div>
        )}
        ...
        <button
          ...
          disabled={!hasValidPrice || isOutOfStock}
          className={cn(
